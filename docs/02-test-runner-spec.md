...
| `find` | mouse/keyboard | Locate the target by plain-English description, with a self-healing locator cache (Phase 3, see §3.6). |
...
### 3.6 `find:` — AI element location with self-healing locators (Phase 3)

`find:` lets an author target an element by **plain-English description** instead of a pixel
coordinate or a hand-captured UIA selector — while keeping later runs deterministic. Resolution is
cost-ordered:

1. **Cached selector (deterministic, no AI).** The session's **locator store**
   (`<session>.locators.yaml`, next to the session file) is checked for the description. A hit
   resolves via UI Automation exactly like a hand-written `uia:` — immune to position/DPI/layout
   drift, no AI call.
2. **AI vision locate.** On a cache miss (or a stale cached selector), the runner captures the
   target window (or screen), asks the AI engine for the element's pixel coordinates
   (`POINT: x,y` answer contract), and maps them to a screen point.
3. **Harvest & cache.** The runner hit-tests the UI Automation tree at that point
   (`ElementFromPoint`, climbing to the nearest ancestor with an `AutomationId` or `Name`),
   **round-trip verifies** the harvested query (searching the window for it must land back on a
   rectangle containing the located point — otherwise it is ambiguous and rejected), then saves it
   to the locator store as an **unapproved candidate**. The action drives the element's center.
   If no durable selector can be harvested (e.g. a WinForms-hosted grid with no accessible
   identity), the AI point is clicked directly and nothing is cached.

```yaml
- human: "Click the Save button in the toolbar"
  machine:
    action: mouse_click
    target: { window: "New Person" }
    find: "the Save button in the form's toolbar"
    verify: { stable: true }
```

**Self-healing for explicit `uia:` targets.** When a command carries **both** `uia:` and `find:`,
the deterministic `uia:` is tried first; if it no longer resolves (the app changed), the runner
heals through the `find:` path and caches the replacement selector — the failure that would have
broken the case instead repairs it and leaves a review trail.

**Approval loop.** Harvested entries start as `approved: false` candidates. They are used
immediately (runs don't block), but every use is logged and the end-of-run summary warns when
candidates were involved. Review them with:

```
uitest locators <session.yaml>                       # list (✓ approved / ? candidate)
uitest locators <session.yaml> --approve "<find>"    # mark one reviewed
uitest locators <session.yaml> --approve-all
uitest locators <session.yaml> --rm "<find>"         # drop it; next run re-locates via AI
```

When a cached selector goes stale and is re-harvested, the new entry returns to candidate state
with a `note:` recording the previous selector, so the reviewer sees what healed and from what.
`uiaprobe at <x> <y>` hit-tests a live point for manual verification of a harvested selector.

The store is plain YAML and is meant to be committed next to the session file: locator churn shows
up in version control as a reviewable diff.
